...
import com.xiangzi.util.URLEncodedUtil;
...
public class HTTPRequest {
	...
	/**
	 * @param follow
	 *            <code>true</code> to automatically follow redirects;
	...
	public HTTPRequest contentType(final String type) {
		return this.header(HttpConstants.HEADER_CONTENT_TYPE, type);
	}
	...
	public HTTPRequest auth(final String authString) {
		return this.header(HttpConstants.HEADER_AUTHORIZATION, authString);
	}
	...
	public HTTPRequest accept(final String type) {
		return this.header(HttpConstants.HEADER_ACCEPT, type);
	}
	...
	public HTTPRequest form(Map<String, Object> form) {
		...
		Iterator<Entry<String, Object>> parts = form.entrySet().iterator();
		if (parts.hasNext()) {
			Entry<String, Object> entry = parts.next();
			formString.append(URLEncodedUtil.encode(entry.getKey().trim()));
			formString.append("=");
			formString.append(URLEncodedUtil.encode(entry.getValue().toString().trim()));
			while (parts.hasNext()) {
				entry = parts.next();
				formString.append("&");
				formString.append(URLEncodedUtil.encode(entry.getKey().trim()));
		...
		logger.debug("http参数--" + formString.toString());

		return this.contentType("application/x-www-form-urlencoded").body(formString.toString());
	}

	public HTTPRequest formSS(Map<String, String> form) {
		StringBuilder formString = new StringBuilder();
		Iterator<Entry<String, String>> parts = form.entrySet().iterator();
		if (parts.hasNext()) {
			Entry<String, String> entry = parts.next();
			formString.append(URLEncodedUtil.encode(entry.getKey()));
			formString.append("=");
			formString.append(URLEncodedUtil.encode(entry.getValue().toString()));
			while (parts.hasNext()) {
				entry = parts.next();
				formString.append("&");
				formString.append(URLEncodedUtil.encode(entry.getKey()));
