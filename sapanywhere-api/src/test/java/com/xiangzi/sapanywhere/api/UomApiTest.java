...
public class UomApiTest extends BaseAnywhereTest {
	...
	@Test
	public void count() {
		int count = uomApi.count(accessToken.getValue());
	...
	public void pager() {
		...
		pager = uomApi.pager(accessToken.getValue(), pager);
		...
		List<Uom> list = (List<Uom>) pager.getList();
		logger.info("list.size:" + list.size());
		logger.info("list.json:" + JSONUtil.toJSONString(list));

		for (Uom uom : list) {
			logger.info("uom.json:" + JSONUtil.toJSONString(uom));
		}
