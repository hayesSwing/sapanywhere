...
public class BundleApiTest extends BaseAnywhereTest {
	...
	@Test
	public void count() {
		int count = bundleApi.count(accessToken.getValue());
	...
	public void pager() {
		...
		pager = bundleApi.pager(accessToken.getValue(), pager);
		...
		List<Bundle> list = (List<Bundle>) pager.getList();
		logger.info("list.size:" + list.size());
		logger.info("list.json:" + JSONUtil.toJSONString(list));

		for (Bundle bundle : list) {
			logger.info("bundle.json:" + JSONUtil.toJSONString(bundle));
		}
