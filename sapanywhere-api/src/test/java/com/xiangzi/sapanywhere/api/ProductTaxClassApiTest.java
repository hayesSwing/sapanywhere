...
public class ProductTaxClassApiTest extends BaseAnywhereTest {

	private ProductTaxClassApi productTaxClassApi;

	@Before
	public void before() throws Exception {
		super.before();
		productTaxClassApi = new ProductTaxClassApi();
	}

	@Ignore
	@Test
	public void count() {
		int count = productTaxClassApi.count(accessToken.getValue());
	...
	public void pager() {
		...
		pager = productTaxClassApi.pager(accessToken.getValue(), pager);
		...
		List<ProductTaxClass> list = (List<ProductTaxClass>) pager.getList();
		logger.info("list.size:" + list.size());
		logger.info("list.json:" + JSONUtil.toJSONString(list));

		for (ProductTaxClass productTaxClass : list) {
			logger.info("productTaxClass.json:" + JSONUtil.toJSONString(productTaxClass));
		}
	...
	public void get() {
		String id = "-1";
		Expand expand = null;
		ProductTaxClass productTaxClass = productTaxClassApi.get(accessToken.getValue(), id, expand);
		logger.info("productTaxClass:" + productTaxClass);
	}
