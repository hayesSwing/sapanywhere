...
public class LengthUnitApiTest extends BaseAnywhereTest {

	private LengthUnitApi lengthUnitApi;

	@Before
	public void before() throws Exception {
		super.before();
		lengthUnitApi = new LengthUnitApi();
	}

	@Ignore
	@Test
	public void count() {
		int count = lengthUnitApi.count(accessToken.getValue());
	...
	public void pager() {
		...
		pager = lengthUnitApi.pager(accessToken.getValue(), pager);
		...
		List<LengthUnit> list = (List<LengthUnit>) pager.getList();
		logger.info("list.size:" + list.size());
		logger.info("list.json:" + JSONUtil.toJSONString(list));

		for (LengthUnit lengthUnit : list) {
			logger.info("lengthUnit.json:" + JSONUtil.toJSONString(lengthUnit));
		}
