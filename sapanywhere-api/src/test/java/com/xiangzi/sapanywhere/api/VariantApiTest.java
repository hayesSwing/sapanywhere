...
public class VariantApiTest extends BaseAnywhereTest {
	...
	@Test
	public void count() {
		int count = variantApi.count(accessToken.getValue());
	...
	public void pager() {
		...
		pager = variantApi.pager(accessToken.getValue(), pager);
		...
		List<Variant> list = (List<Variant>) pager.getList();
		logger.info("list.size:" + list.size());
		logger.info("list.json:" + JSONUtil.toJSONString(list));

		for (Variant variant : list) {
			logger.info("variant.json:" + JSONUtil.toJSONString(variant));
		}
