...
public class VendorApiTest extends BaseAnywhereTest {
	...
	@Test
	public void count() {
		int count = vendorApi.count(accessToken.getValue());
	...
	public void pager() {
		...
		pager = vendorApi.pager(accessToken.getValue(), pager);
		...
		List<Vendor> list = (List<Vendor>) pager.getList();
		logger.info("list.size:" + list.size());
		logger.info("list.json:" + JSONUtil.toJSONString(list));

		for (Vendor vendor : list) {
			logger.info("vendor.json:" + JSONUtil.toJSONString(vendor));
		}
	...
	public void get() {
		...
		Expand expand = null;
		expand = new Expand("*");
		Vendor vendor = vendorApi.get(accessToken.getValue(), id, expand);
