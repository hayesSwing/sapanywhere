...
public class ProductBrandApiTest extends BaseAnywhereTest {
	...
	public void before() throws Exception {
		super.before();
		productBrandApi = new ProductBrandApi();
	}

	@Ignore
	@Test
	public void count() {
		int count = productBrandApi.count(accessToken.getValue());
	...
	public void pager() {
		...
		pager = productBrandApi.pager(accessToken.getValue(), pager);
		...
		List<ProductBrand> list = (List<ProductBrand>) pager.getList();
		logger.info("list.size:" + list.size());
		logger.info("list.json:" + JSONUtil.toJSONString(list));

		for (ProductBrand productBrand : list) {
			logger.info("productBrand.json:" + JSONUtil.toJSONString(productBrand));
		}
